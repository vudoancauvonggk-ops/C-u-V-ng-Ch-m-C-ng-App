...
// 1. Add imports for dnd-kit and GripVertical
...
// 2. Add SortableTeacherRow component
if (!content.includes('const SortableTeacherRow')) {
  console.log('Adding SortableTeacherRow...');
...
// 3. Add sensors & teacherOrder state to AdminDashboard
if (!content.includes('const [teacherOrder, setTeacherOrder]')) {
  console.log('Adding state hooks...');
  const stateHooks = `
  ...
  const handleTeacherDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const activeTeacher = teachers.find(t => t.id === active.id);
      ...
      const oldIndex = orderedTeachers.findIndex((t) => t.id === active.id);
      const newIndex = orderedTeachers.findIndex((t) => t.id === over.id);
      
      const newOrdered = arrayMove(orderedTeachers, oldIndex, newIndex);
      const newIds = newOrdered.map(t => t.id);
...
if (!content.includes('<DndContext sensors={sensors}')) {
  console.log('Replacing table body...');
  
  // Use regex to match the exact block to replace to avoid whitespace issues
  const newTableFull = `
                  <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleTeacherDragEnd}>
                    <SortableContext items={teachers.map(t => t.id)} strategy={verticalListSortingStrategy}>
                      {[...teachers]
                        .filter(t => handleQueryFilter(t, ['id', 'name', 'phone', 'email']))
                        .sort((a, b) => {
  ...
  if (regex.test(content)) {
     ...
     console.log('Regex did not match the table body');
  }
}

fs.writeFileSync('src/components/AdminDashboard.tsx', content);
console.log('Patched AdminDashboard.tsx for teacher sorting');
