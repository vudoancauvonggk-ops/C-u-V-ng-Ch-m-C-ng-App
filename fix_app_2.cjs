...
const regex = /\/\/ Startup useEffect: Fetch PostgreSQL state on initialization & poll for updates to keep frontend in perfect sync with the DB\s*useEffect\(\(\) => \{\s*let currentServerVersion: string \| null = null;[\s\S]*?\/\/ Set up real-time polling every 5 seconds \(auto-refresh cross-device\)\s*const intervalId = setInterval\(fetchLatestState, 5000\);/m;
