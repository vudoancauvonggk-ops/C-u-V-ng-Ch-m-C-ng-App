...
const searchString = `{(() => {
                                        const items: { schedules?: any[], attendance?: any }[] = [];
                                        ...
                                        Object.keys(groupedSchedules).forEach(key => {
                                           ...
                                           if (atts.length > 0) {
                                             atts.forEach(a => items.push({ schedules: schs, attendance: a }));
                                           } else {
                                        ...
                                        Object.keys(groupedAttendance).forEach(key => {
                                           if (!groupedSchedules[key]) {
                                             groupedAttendance[key].forEach(a => items.push({ attendance: a }));
                                           }
                                        });
                                          ...
                                          return (
                                            <tr key={idx} className={\`\${att && att.isFlagged ? 'bg-red-50/50' : 'hover:bg-slate-50'}\`}>
                                              <td className="p-2 pl-4">
                                                <div className="font-bold text-slate-800 text-xs">Ca {getSessionShort(firstSch ? firstSch.session : att.session)}</div>
                                                {att && <div className="text-[10px] text-slate-500 font-mono">{att.checkInTime}</div>}
                                              </td>
                                              <td className="p-2">
                                                <div className="font-semibold text-slate-700 text-xs">{getSchoolName(firstSch ? firstSch.schoolId : att.schoolId, firstSch || att)}</div>
...
if (content.includes(searchString)) {
  fs.writeFileSync('src/components/AdminDashboard.tsx', content.replace(searchString, replaceString));
