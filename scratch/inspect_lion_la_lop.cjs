...
conn.on('ready', () => {
  ...
  conn.exec(`cd /app && npx tsx -r dotenv/config -e "
    ...
    async function run() {
      ...
      const targetSchools = schs.filter(s => s.name === 'Lá' || s.name === 'Lion' || s.name === 'Lớp');
      ...
      filteredAtts.forEach(a => {
        ...
        console.log('AttId:', a.id, '| School:', sch?.name, '(', a.schoolId, ')', '| Class:', cl ? cl.name : 'Unknown', '(', a.classId, ')', '| Teacher:', t ? t.name : 'Unknown', '| Date:', a.date, '| Periods:', a.periods);
      });
