...
conn.on('ready', () => {
  ...
  conn.exec(`cd /app && npx tsx -r dotenv/config -e "
    ...
    async function run() {
      ...
      const msYen = tchs.find(t => t.name.includes('Yên'));
      console.log('Ms. Yên ID:', msYen ? msYen.id : 'Not Found');

      if (msYen) {
        const skdsYen = skds.filter(s => s.teacherId === msYen.id);
        console.log('=== Schedules of Ms. Yên ===');
        skdsYen.forEach(s => {
